...
class PurioSupabaseUser extends BaseAuthUser {
  PurioSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> purioSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PurioSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
